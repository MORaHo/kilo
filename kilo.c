...
int editorRowCxToRx(erow *row, int cx)  {
    ...
    for (j = 0; j < cx; j++)  {
        if(row->chars[j] == '\t')
            rx += (KILO_TAB_STOP - 1) - (rx % KILO_TAB_STOP); //adds the number of spaces generated by the tab to our rendered position
        rx++;
    }
    return rx;
}
...
void editorUpdateRow(erow *row) {
    ...
    free(row->render);
    row->render = malloc(row->size+tabs*(KILO_TAB_STOP-1) + 1); //allocating space for tabs, since \t already occupies one byte, all we need it to 
    ...
    for (j = 0; j<row->size; j++) {
        ...
        if (row->chars[j] == '\t') { //because tab moves the mouse cursor, just like \r\n and doesn't effectively add any characters, so it never removes the text behind it, so we have to render it on our own.
            ...
            row->render[idx++] = ' ';
            while (idx % KILO_TAB_STOP != 0) row->render[idx++] = ' '; //we never reset idx because we increase idx as we count, so even if it is 0, after the check it is ticked up and we have to add up to another7 to end the loop
        ...
        if (filerow >= E.numrows) { //any line we are drawing that is over the text file length will have a ~ at the start
            ...
            if (E.numrows == 0 && y == E.screenrows / 3) { //writing welcome message branch
                ...
                int welcomelen = snprintf(welcome, sizeof(welcome),
                    "Kilo editor -- version %s", KILO_VERSION); //adds KILO_VERSION to the string.
...
char *editorPrompt(char *prompt, void (*callback)(char *, int))    { //callback is a is a variable which in this case will be a function that passes (char*, int), and in this case is also a pointer, and outputs void.
    ...
    while(1)    {
        ...
        if (c == DEL_KEY || c == CTRL_KEY('h') || c == BACKSPACE)   {
            ...
            if (buflen != 0)    {
                editorSetStatusMessage("");
                if (callback) callback(buf, c); //we call this to end the search.
                return buf;
            }
